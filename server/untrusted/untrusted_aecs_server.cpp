...
Status AecsServiceImpl::TaRemoteCall(ServerContext* context,
  ...
  // Call the trusted function
  std::string name = req->function_name();
  TEE_LOG_INFO("TA remote call: %s", name.c_str());
  TeeErrorCode ret = enclave_->TeeRun("TeeTaRemoteCall", *req, res);
  RETURN_ERROR(ret, "Fail to call TA remote function");

  // Return server's RA report and identity public key
  UnifiedAttestationAuthReport* server_auth = res->mutable_auth_report();
  ret = GetServerRaAuthentication(server_auth);
  RETURN_ERROR(ret, "Fail to load server RA report");
